...
from mx.Tools import freeze, reval, NotGiven
...
class Entry:
    ...
    def parse(self,text,locals,globals):
        ...
        if not isinstance(text, (str, unicode)):
            text = str(text)
        text = self.pre_process(text,locals,globals)
...
class UnicodeEntry(Entry):

    """ Converts the value to a Unicode object.
    """
    # Encoding to expect
    encoding = 'utf-8'
    ...
    def __init__(self, default=NotGiven, encoding='utf-8'):

        self.encoding = encoding
        self.converters = (self.convert, )
        Entry.__init__(self, default)
...
class ConfigNamespace(Namespace.Namespace):
    ...
    def _data_dictionary(self):
        ...
        """ Return the parsed data in form of a dictionary
            of dictionaries (one for each section).

            Unlike ._dictionary() this also converts values which are
            ConfigNamespace instances to dictionaries, so no traces of
            ConfigNamespaces should be left in the resulting
            dictionary.
